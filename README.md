...
> 作为个人图床使用，通过 [PicGo](https://github.com/Molunerfinn/PicGo) 工具实现一键上传图片到 GitHub 仓库，方便快捷地管理图片资源。

---

## 📌 用途说明

用于存储上传的图片资源，适用于博客、笔记、文档、聊天等场景中的图片外链需求。基于 GitHub + PicGo 搭建，可以：
...
- ✅ 安装 [PicGo](https://github.com/Molunerfinn/PicGo/releases)（推荐最新版本）

---

### 2. 获取 GitHub Token

GitHub 从 2021 年起不再支持使用用户名密码进行 API 操作，因此需要创建一个 Personal Access Token。
...
4. 设置 Token 的过期时间（建议选择合适的有效期，如90天）
5. 点击 `Generate token`
...
| `自定义域名` | `https://cdn.jsdelivr.net/gh/username/image-bed@main/`（可选） | 通过 jsDelivr 加速访问 |
...
- **截图上传**：在 PicGo 设置中可开启截图上传快捷键（默认 `Ctrl + Shift + P`）
...
后续可直接粘贴到 Markdown、博客或文档中使用。

---

## ⚠️ 注意

- GitHub 免费账户的图床访问速度取决于 CDN（如使用 jsDelivr）。
- 图片命名请避免重复，PicGo 默认使用时间戳命名。
- 不建议上传敏感或隐私图片。
...
- A：检查 Token 是否正确、是否过有效期、仓库名是否填写完整（格式为 `username/xxx`）、分支是否正确。

### Q：图片无法访问？

- A：检查仓库是否为公开仓库；若使用 jsDelivr，请确认路径是否正确（如 `images/xxx.png`）。
